...
def check_werte_random(section):
    try:
	    ...
	    for line in lines:
			...
			"0x01": (State.CRIT, "Critical Warning type: 0x01 - Temperature is over or under temperature threshold"),
			"0x02": (State.CRIT, "Critical Warning type: 0x02 - NVM subsystem reliability has been degraded due to significant"),
			"0x03": (State.CRIT, "Critical Warning type: 0x03 - The media has been placed in read-only mode"),
			"0x04": (State.CRIT, "Critical Warning type: 0x04 - The volatile memory backup device has failed"),
			"0x05": (State.CRIT, "Critical Warning type: 0x05 - The Persistent Memory Region has become read-only or unreliable"),
			}
